...
function SubscriptionBanner({ request, onOpen }) {
  const [subscription, setSubscription] = useState(null);
  useEffect(() => { let active = true; request('/api/suscripcion').then((data) => active && setSubscription(data)).catch(() => {}); return () => { active = false; }; }, [request]);
  if (!subscription || subscription.estado === 'activa' || subscription.tipo === 'gratuita') return null;
  const text = subscription.estado === 'prueba' ? 'Tu prueba está activa. Revisá la fecha de finalización.' : subscription.estado === 'en_gracia' ? 'Tu cobro no se acreditó. Tenés 7 días de gracia mientras Mercado Pago reintenta.' : subscription.estado === 'sin_suscripcion' ? 'Necesitás una suscripción para administrar complejos y recibir reservas.' : 'Tu suscripción no está activa. Tus complejos quedan en modo lectura.';
  return <button className={`subscription-banner subscription-banner--${subscription.estado}`} type="button" onClick={onOpen}><span>{text}</span><strong>Ver suscripción <Icon name="arrow" size={15} /></strong></button>;
}
