...
const emptyCourt = { nombre: '', deporte: 'Fútbol 5', descripcion: '', indoor: false, requiere_sena: true };
...
function ComplexesManager({ complexes, reload, request }) {
  ...
  const selectCourt = (court) => { setSelectedCourt(court); setCourtEdit({ nombre: court.nombre || '', deporte: court.deporte || 'Fútbol 5', descripcion: court.descripcion || '', indoor: Boolean(court.indoor), requiere_sena: court.requiere_sena !== false }); };
  ...
  const deleteComplex = async () => { if (saving || !window.confirm(`¿Eliminar definitivamente “${activeComplex.nombre}” y todas sus canchas? El historial de reservas se conservará.`)) return; setSaving(true); try { await request(`/api/admin/complejos/${activeComplex.id}`, { method: 'DELETE' }); setSelectedComplex(null); setSelectedCourt(null); await reload(); show('Complejo eliminado definitivamente.'); } catch (error) { show(error.message, 'error'); window.alert(`No se pudo eliminar el complejo: ${error.message}`); } finally { setSaving(false); } };
  ...
  return <section className="admin-bookings-section admin-complexes">
    <div className="admin-section-heading"><h2>Complejos y canchas</h2></div>
    <form className="admin-form admin-complex-create" onSubmit={createComplex}>
      <div className="admin-complex-create__heading"><h3>Agregar nuevo complejo</h3><p>Completá los datos del lugar y su primera cancha.</p></div>
      <div className="admin-form__title"><span>1</span><div><h3>Datos del complejo</h3><p>La ubicación, el contacto y la foto se comparten entre sus canchas.</p></div></div>
      <div className="admin-complex-fields"><ComplexFields value={complexForm} onChange={setComplexForm} /></div>
      <PhotoField currentUrl="" file={createPhoto} onFile={setCreatePhoto} onRemove={() => setCreatePhoto(null)} />
      <div className="admin-form__title"><span>2</span><div><h3>Primera cancha</h3><p>Después vas a poder sumar todas las que necesites.</p></div></div>
      <div className="admin-court-fields"><CourtFields value={firstCourt} onChange={setFirstCourt} /></div>
      <Button type="submit" disabled={saving}>{saving ? 'Creando complejo…' : 'Crear complejo y cancha'}</Button>
    </form>
    {message && <p className={messageType === 'success' ? 'form-success' : 'form-error'}>{message}</p>}
    <div className="admin-complex-list">{complexes.map((complex) => { const complexSports = complex.canchas.map((court) => court.deporte); const theme = getComplexTheme(complexSports); return <button className={`admin-complex-card sport-theme--${theme}${activeComplex?.id === complex.id ? ' is-selected' : ''}`} type="button" key={complex.id} onClick={() => selectComplex(complex)}>{complex.foto_url ? <img src={complex.foto_url} alt="" /> : <div className="admin-complex-card__placeholder"><PitchMark /></div>}<AdminSportStripes sports={complexSports} /><span><strong>{complex.nombre}</strong><small>{complex.ciudad}, {complex.provincia}</small><b>{complex.canchas.length} {complex.canchas.length === 1 ? 'cancha' : 'canchas'}</b></span></button>; })}</div>
    {activeComplex && <div className="admin-complex-workspace">
      <form className="admin-form admin-complex-edit" onSubmit={saveComplex}>
        <div className="admin-section-heading admin-complex-edit__heading"><div><h3>Editar {activeComplex.nombre}</h3><p>Actualizá los datos que comparten sus canchas.</p></div><Button className="admin-danger" variant="secondary" size="sm" type="button" onClick={deleteComplex}>Eliminar complejo</Button></div>
        <div className="admin-complex-fields"><ComplexFields value={complexEdit} onChange={setComplexEdit} /></div>
        <PhotoField currentUrl={complexEdit.foto_url} file={editPhoto} onFile={setEditPhoto} onRemove={() => { setEditPhoto(null); setComplexEdit({ ...complexEdit, foto_url: '' }); }} />
        <Button variant="secondary" type="submit" disabled={saving}>Guardar complejo</Button>
      </form>
      <MercadoPagoSettings key={activeComplex.id} complex={activeComplex} request={request} />
      <section className="admin-courts-panel">
        <div className="admin-courts-panel__heading"><h3>Canchas del complejo</h3><p>Agregá una cancha o elegí una creada para editarla.</p></div>
        <form className="admin-form admin-add-court" onSubmit={createCourt}><div><h3>Agregar otra cancha</h3><p>Quedará disponible para configurar sus horarios después de crearla.</p></div><div className="admin-court-fields"><CourtFields value={newCourt} onChange={setNewCourt} /></div><Button type="submit">Agregar cancha</Button></form>
        <div className="admin-court-list-section"><h3>Canchas creadas</h3><div className="admin-court-list">{activeComplex.canchas.map((court) => <button className={`admin-court-card sport-theme--${getSportTheme(court.deporte)}${selectedCourt?.id === court.id ? ' is-selected' : ''}`} type="button" key={court.id} onClick={() => selectCourt(court)}><strong>{court.nombre}</strong><small>{court.deporte} · {court.indoor ? 'Indoor' : 'A cielo abierto'}</small><span className={`admin-court-card__deposit${court.requiere_sena === false ? ' is-free' : ''}`}>{court.requiere_sena === false ? 'Sin seña' : 'Seña requerida'}</span></button>)}</div></div>
        {selectedCourt && <section className="admin-court-editor"><form className="admin-form admin-court-edit" onSubmit={saveCourt}><div className="admin-section-heading"><div><h3>Editar {selectedCourt.nombre}</h3><p>Datos, horarios, precios y excepciones de esta cancha.</p></div><Button className="admin-danger" variant="secondary" size="sm" type="button" onClick={deleteCourt}>Eliminar cancha</Button></div><div className="admin-court-fields"><CourtFields value={courtEdit} onChange={setCourtEdit} /></div><Button variant="secondary" type="submit">Guardar cancha</Button></form><SlotEditor key={selectedCourt.id} court={selectedCourt} request={request} /></section>}
      </section>
    </div>}
  </section>;
}
...
function SubscriptionStatus({ subscription, onCancel, busy }) {
  if (!subscription || subscription.estado === 'sin_suscripcion') return <section className="subscription-card subscription-card--warning"><div><span className="section-kicker">ACCESO COMERCIAL</span><h2>Necesitás una suscripción</h2><p>Elegí un plan para crear complejos y recibir reservas.</p></div><a className="primary-button" href="/planes">Ver planes <Icon name="arrow" size={16} /></a></section>;
  const stateLabel = { pendiente: 'Pendiente', prueba: 'En prueba', activa: 'Activa', en_gracia: 'En período de gracia', vencida: 'Vencida', anulada: 'Anulada' }[subscription.estado] || subscription.estado;
  const date = subscription.estado === 'prueba' ? subscription.prueba_finaliza_at : subscription.estado === 'en_gracia' ? subscription.gracia_hasta_at : subscription.proximo_cobro_at;
  const dateLabel = subscription.estado === 'prueba' ? 'Fin de prueba' : subscription.estado === 'en_gracia' ? 'Fin de gracia' : 'Próximo cobro';
  return <section className={`subscription-card subscription-card--${subscription.estado}`}><div className="subscription-card__top"><div><span className="section-kicker">SUSCRIPCIÓN</span><h2>{subscription.plan?.nombre || 'Suscripción'}</h2><p>{subscription.tipo === 'gratuita' ? 'Acceso gratuito administrado por NEW MATCH.' : `${Number(subscription.plan?.precio_ars || 0).toLocaleString('es-AR', { style: 'currency', currency: 'ARS', maximumFractionDigits: 0 })} por mes · 0% de comisión por reserva.`}</p></div><span className="subscription-state">{stateLabel}</span></div><dl className="subscription-details"><div><dt>{dateLabel}</dt><dd>{date ? new Intl.DateTimeFormat('es-AR', { dateStyle: 'medium' }).format(new Date(date)) : 'A confirmar por Mercado Pago'}</dd></div><div><dt>Límites</dt><dd>{subscription.complexes_used}/{subscription.plan?.max_complejos} sedes · {subscription.courts_used}/{subscription.plan?.max_canchas} canchas</dd></div></dl>{subscription.plan?.code !== 'pro' && subscription.tipo === 'mercadopago' && ['prueba', 'activa', 'en_gracia'].includes(subscription.estado) && <a className="secondary-button" href="/planes">Mejorar a Pro</a>}{subscription.tipo !== 'gratuita' && subscription.estado !== 'anulada' && <Button className="subscription-cancel" variant="secondary" type="button" disabled={busy} onClick={onCancel}>Anular suscripción</Button>}<p className="subscription-note">Al anular, el acceso termina inmediatamente, los complejos se ocultan y no hay devolución proporcional. Para volver, necesitás crear una nueva suscripción; la prueba no se repite.</p></section>;
}
...
function SubscriptionCancelDialog({ target, busy, onClose, onConfirm }) {
  ...
  useEffect(() => {
    ...
    const focusFirst = () => dialogRef.current?.querySelector('textarea, button:not([disabled])')?.focus();
    const timer = window.setTimeout(focusFirst, 0);
    const handleKeyDown = (event) => {
      if (event.key === 'Escape' && !busy) onClose();
      if (event.key !== 'Tab' || !dialogRef.current) return;
      const controls = [...dialogRef.current.querySelectorAll('textarea, button:not([disabled])')];
      if (!controls.length) return;
      const currentIndex = controls.indexOf(document.activeElement);
      if (event.shiftKey && currentIndex <= 0) { event.preventDefault(); controls.at(-1).focus(); }
      if (!event.shiftKey && currentIndex === controls.length - 1) { event.preventDefault(); controls[0].focus(); }
    };
  ...
  return <div className="subscription-cancel-dialog" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose(); }}><section className="subscription-cancel-dialog__panel" role="dialog" aria-modal="true" aria-labelledby="cancel-subscription-title" aria-describedby="cancel-subscription-description" ref={dialogRef}><div className="subscription-cancel-dialog__heading"><div><h3 id="cancel-subscription-title">¿Anular {targetName}?</h3><p id="cancel-subscription-description">El acceso termina inmediatamente, los complejos dejan de estar publicados y no hay devolución proporcional.</p></div><Button variant="secondary" size="sm" type="button" disabled={busy} onClick={onClose}>Cerrar</Button></div><ul className="subscription-cancel-dialog__consequences"><li>Se cancela la recurrencia en Mercado Pago.</li><li>Para volver, será necesario crear una suscripción nueva.</li><li>La prueba gratuita no vuelve a estar disponible.</li></ul><details className="subscription-cancel-dialog__reason"><summary>Agregar motivo administrativo <span>(opcional)</span></summary><label className="subscription-cancel-dialog__label">Motivo administrativo<textarea value={reason} maxLength={500} disabled={busy} onChange={(event) => setReason(event.target.value)} placeholder="Ej.: Prueba de anulación" /></label></details><div className="subscription-cancel-dialog__actions"><Button variant="secondary" type="button" disabled={busy} onClick={onClose}>Volver</Button><Button className="subscription-cancel-dialog__confirm" type="button" disabled={busy} onClick={() => onConfirm(reason)}>{busy ? 'Anulando…' : 'Anular suscripción'}</Button></div></section></div>;
}

function SubscriptionManager({ isSuperadmin, request }) {
  const [subscription, setSubscription] = useState(null); const [items, setItems] = useState([]); const [email, setEmail] = useState(''); const [note, setNote] = useState(''); const [message, setMessage] = useState(''); const [busy, setBusy] = useState(false); const [cancelTarget, setCancelTarget] = useState(undefined);
  ...
  const cancel = async (reason) => {
    ...
    try { await request(target ? `/api/superadmin/suscripciones/${target.id}/anular` : '/api/suscripcion/anular', { method: 'POST', body: JSON.stringify({ motivo: reason }) }); setMessage('La suscripción fue anulada correctamente.'); setCancelTarget(undefined); await load(); } catch (error) { setMessage(error.message); } finally { setBusy(false); }
  };
  const createFree = async (event) => { event.preventDefault(); setBusy(true); setMessage(''); try { await request('/api/superadmin/suscripciones/gratuita', { method: 'POST', body: JSON.stringify({ email, nota: note }) }); setEmail(''); setNote(''); setMessage('Usuario gratuito creado. Si todavía no tiene cuenta, quedará invitado hasta su primer ingreso con Google.'); await load(); } catch (error) { setMessage(error.message); } finally { setBusy(false); } };
  return <section className="admin-bookings-section subscription-page"><div className="admin-section-heading"><div><span className="section-kicker">CONTROL COMERCIAL</span><h2>Suscripciones</h2></div></div>{message && <p className={message.includes('correctamente') || message.includes('creado') ? 'form-success' : 'form-error'} role="status">{message}</p>}{!isSuperadmin && <SubscriptionStatus subscription={subscription} onCancel={() => setCancelTarget({ target: null })} busy={busy} />}{isSuperadmin && <><section className="subscription-card"><div><span className="section-kicker">USUARIOS GRATUITOS</span><h2>Dar acceso gratuito</h2><p>Usan los límites del plan Estándar: una sede y hasta seis canchas. No tienen prueba, cobros ni vencimiento automático.</p></div><form className="subscription-free-form" onSubmit={createFree}><Input type="email" required placeholder="correo@ejemplo.com" value={email} onChange={(event) => setEmail(event.target.value)} /><Input placeholder="Nota administrativa (opcional)" value={note} onChange={(event) => setNote(event.target.value)} /><Button type="submit" disabled={busy}>Crear usuario gratuito</Button></form></section><div className="subscription-list">{items.map((item) => <article className="subscription-list__item" key={item.id}><div><strong>{item.usuario_nombre || 'Pendiente de ingreso'}</strong><small>{item.email} · {item.plan?.nombre} · {item.tipo}</small></div><div><span className={`subscription-state subscription-state--${item.estado}`}>{item.estado.replace('_', ' ')}</span><small>{item.complexes_used}/{item.plan?.max_complejos} sedes · {item.courts_used}/{item.plan?.max_canchas} canchas</small></div>{item.estado !== 'anulada' && <Button className="subscription-cancel" variant="secondary" size="sm" type="button" disabled={busy} onClick={() => setCancelTarget({ target: item })}>Anular</Button>}</article>)}</div></>}{cancelTarget && <SubscriptionCancelDialog target={cancelTarget.target} busy={busy} onClose={() => !busy && setCancelTarget(undefined)} onConfirm={cancel} />}</section>;
}

export default function PanelAdmin() {
  const { data: session, isPending } = useSessionWithFallback();
  const sessionUserId = session?.user?.id;
  const [profile, setProfile] = useState(null); const [subscription, setSubscription] = useState(null); const [bookings, setBookings] = useState([]); const [complexes, setComplexes] = useState([]); const [admins, setAdmins] = useState([]); const [filterDate, setFilterDate] = useState(''); const [activeSection, setActiveSection] = useState('overview'); const [error, setError] = useState('');
  ...
  useEffect(() => {
    ...
    request('/api/admin/session').then((data) => {
      if (active) { setProfile(data.user); setSubscription(data.suscripcion); }
    }).catch((requestError) => {
      if (!active) return;
  ...
  const cancelBooking = async (id) => { if (!window.confirm('¿Querés cancelar este turno? Si la seña está pendiente, el horario se liberará de inmediato.')) return; setError(''); try { await request(`/api/admin/reservas/${id}`, { method: 'DELETE' }); await reload(); } catch (requestError) { setError(requestError.message); window.alert(`No se pudo cancelar la reserva: ${requestError.message}`); } };
