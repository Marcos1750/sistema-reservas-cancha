...
function SubscriptionManager({ isSuperadmin, request }) {
  ...
  const cancel = async (target) => {
    ...
    try { await request(target ? `/api/superadmin/suscripciones/${target.id}/anular` : '/api/suscripcion/anular', { method: 'POST', body: JSON.stringify({ motivo: reason }) }); setMessage('La suscripción fue anulada correctamente.'); await load(); } catch (error) { setMessage(error.message); } finally { setBusy(false); }
  };
